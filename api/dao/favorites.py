   ...
   *
    ...
    def add(self, user_id, movie_id):
        ...
        def add_to_favorites(tx,user_id,movie_id):
            ...
            MERGE (u)-[r:HAS_FAVORITE]->(m) ON CREATE SET r.createdAt=datetime()
            RETURN m{
                .*,
                favorite:true
            } AS movie
            """
            row=tx.run(cypher,userId=user_id,movieId=movie_id).single()
            if row == None:
    ...
    def remove(self, user_id, movie_id):
        ...
        # TODO: Execute the transaction function within a Write Transaction
        # TODO: Return movie details and `favorite` property
        def remove_from_favorties(tx,user_id,movie_id):
            cypher="""
            ...
            return row.get("movie")
        with self.driver.session() as session:
            return session.execute_write(remove_from_favorties,user_id,movie_id) 
